...
def load_img(filepath, data_transform,class_name):
    ...
    visualization,max_pos1,max_pos2 = show_cam_on_image(img.astype(dtype=np.float32) / 255.,
                                      grayscale_cam,
                                      use_rgb=True)
    info.append((filepath,max_pos1,max_pos2))
    print((filepath,max_pos1,max_pos2))
    image_bgr = cv2.cvtColor(visualization, cv2.COLOR_RGB2BGR)
    cv2.rectangle(image_bgr, (int(max_pos1[1]*300), int(max_pos1[0]*300)), (int(max_pos1[1]*300 + 0.4*300), int(max_pos1[0]*300 + 0.4*300)), (255,0,0), 2)
    cv2.rectangle(image_bgr, (int(max_pos2[1] * 300), int(max_pos2[0] * 300)),(int(max_pos2[1] * 300 + 0.4 * 300), int(max_pos2[0] * 300 + 0.4 * 300)), (0, 255, 0), 2)
    cv2.imwrite(join("heatmap2",filepath.strip().split("\\")[-1]),image_bgr)
...
from utils_max2 import GradCAM, show_cam_on_image, center_crop_img
...
info=[]
header=['img_name','max_location1',"max_location2"]

for i, (datas) in enumerate(train_loader, ):
    pass
for i, (datas) in enumerate(test_loader,):
    pass
with open('localinfo\\local_info_max2.csv', 'w', encoding='utf-8') as file_obj:
