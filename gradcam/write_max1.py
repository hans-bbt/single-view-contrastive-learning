...
def load_img(filepath, data_transform,class_name):
    ...
    visualization,max_pos = show_cam_on_image(img.astype(dtype=np.float32) / 255.,
                                      grayscale_cam,
                                      use_rgb=True)
    info.append((filepath,max_pos))
    print((filepath,max_pos))
    # image_bgr = cv2.cvtColor(visualization, cv2.COLOR_RGB2BGR)
    # cv2.rectangle(image_bgr, (int(max_pos[1]*300), int(max_pos[0]*300)), (int(max_pos[1]*300 + 0.4*300), int(max_pos[0]*300 + 0.4*300)), (255,0,0), 2)
    # cv2.imwrite(join("heatmap",filepath.strip().split("\\")[-1]),image_bgr)
...
import csv
import model_repAlexnet
from utils_max1 import GradCAM, show_cam_on_image, center_crop_img
...
info=[]
header=['img_name','max_location1']

for i, (datas) in enumerate(train_loader, ):
    pass
for i, (datas) in enumerate(test_loader,):
    pass
with open('localinfo\\local_info_max1.csv', 'w', encoding='utf-8') as file_obj:
