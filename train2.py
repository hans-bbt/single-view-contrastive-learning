'''compare with random and random'''
import argparse
import torch
...
import csv
from model_selfcontrast_EfficientNet import contrast_model
...
parser.add_argument("--path_model", type=str, default='store\\pth_\\self_contrast_loss_maxandrand_efficientnet_n500.pth', help="")
parser.add_argument("--path_loss",  type=str, default="store\\loss_csv\\self_contrast_loss_maxandrand_efficientnet_n500.csv", help="")
parser.add_argument("--path_false", type=str, default="store\\false_info\\self_contrast_loss_maxandrand_efficientnet_n500.csv", help="")
...
model=contrast_model(pretrained=True).to(opt.cuda)
...
with open("gradcam/localinfo/local_info.csv",'r',encoding="utf-8") as file_obj:
...
for epoch in range(0,opt.EPOCHS):
    ...
    for datas, labels,filename in loop:
        ...
        for x in filename:
            ...
            max_regions.append([eval(result[0][1])])
    ...
    if epoch==99 or epoch==199 or epoch==299 or epoch==399:
        torch.save(model.state_dict(), opt.path_model.replace("500",str(epoch)))
        print("save model")
